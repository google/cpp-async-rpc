...
#ifndef LASR_REGISTRY_H_
#define LASR_REGISTRY_H_
...
class dynamic_subclass_registry
 ...
 private:
  lasr::flat_set<std::string_view> dynamic_subclass_set_;
};
...
class dynamic_encoder_registry : public singleton<dynamic_encoder_registry<S>> {
 public:
  using encoder_function_type = void (*)(S&, const ::lasr::dynamic_base_class&);
  struct info {
  ...
  void register_class(std::string_view class_name) {
    encoder_function_type f = [](S& s, const ::lasr::dynamic_base_class& o) {
 ...
 private:
  lasr::flat_map<std::string_view, info> encoder_info_map_;
};
...
class dynamic_decoder_registry : public singleton<dynamic_decoder_registry<S>> {
 public:
  using decoder_function_type = void (*)(S&, ::lasr::dynamic_base_class&);
  struct info {
  ...
  template <typename T>
  void register_class(std::string_view class_name) {
    decoder_function_type f = [](S& s, ::lasr::dynamic_base_class& o) {
 ...
 private:
  lasr::flat_map<std::string_view, info> decoder_info_map_;
};
...
class dynamic_object_factory : public singleton<dynamic_object_factory> {
 public:
  using factory_function_type = ::lasr::dynamic_base_class* (*)();
  struct info {
  ...
  std::string_view register_class(std::string_view class_name) {
    ...
    factory_function_type f = []() {
      return static_cast<::lasr::dynamic_base_class*>(new T());
    };
 ...
 private:
  lasr::flat_map<std::string_view, info> factory_function_map_;
};
...
std::enable_if_t<is_dynamic_v<T>, bool> check_dynamic_compatibility(void* obj) {
  return dynamic_subclass_registry<T>::get().is_subclass(
      (static_cast<::lasr::dynamic_base_class*>(obj))->portable_class_name());
}
