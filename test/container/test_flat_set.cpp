...
TEST_CASE( "flat_set tests" ) {
	...
	SECTION( "empty true for empty set" ) {
		REQUIRE( s1.empty() );
	}

	SECTION( "empty false for non-empty set" ) {
	...
	SECTION( "inserting individual items works" ) {
		...
		REQUIRE_FALSE( s2.insert(4).second );
		REQUIRE( s3.insert(5).second) ;

		REQUIRE( vector(s1.begin(), s1.end()) == (vector{3}) );
		REQUIRE( vector(s2.begin(), s2.end()) == (vector{3, 4}) );
		REQUIRE( vector(s3.begin(), s3.end()) == (vector{3, 4, 5}) );
	}
	...
	SECTION( "erasing individual items with iterators works" ) {
		...
		REQUIRE( vector(s3.begin(), s3.end()) == (vector{3}) );
	}
	...
	SECTION( "erasing a range works" ) {
		...
		REQUIRE( vector(s3.begin(), s3.end()) == (vector{4}) );
	}
